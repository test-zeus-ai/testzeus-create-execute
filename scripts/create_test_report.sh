...
testzeus test-run-group execute-and-monitor --name "$TEST_RUN_NAME" --test-ids "$ALL_TEST_IDS" --interval 60 --filename "$REPORT_FILENAME" --output-dir '.' --notification-channels "$NOTIFICATION_CHANNELS"